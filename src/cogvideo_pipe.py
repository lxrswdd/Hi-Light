...
from src.cogvideo_ddim import cog_ddim_step
...
class CogVideoXVideoToVideoPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def prepare_latents(
        self,
        video: Optional[torch.Tensor] = None, ## 输入 torch.Size([1, 3, 49, 480, 720])
        batch_size: int = 1,
        ...
        shape = ( ## (1, 13, 16, 60, 90)
            ...
            height // self.vae_scale_factor_spatial, ## 8倍降采样
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            ...
            for i, t in enumerate(timesteps):
                ...
                k = 2
                lbd = 1 - (i / (num_inference_steps - 1)) ** k
                detail_lbd = 1 - (i / (num_inference_steps - 1))
                ...
                if lbd > 0.2:
                    ...
                    consist_target = consist_target + detail_lbd * detail_diff
                    ...
                    fusion_target = self.vae.encode(fusion_target).latent_dist.mode() * self.vae_scaling_factor_image
                    fusion_target = fusion_target.to(fusion_target.dtype).permute(0, 2, 1, 3, 4) 
                
                    ## Progresive Light Fusion
                    output = cog_ddim_step(self.scheduler,noise_pred, t, latents,fusion_target,**extra_step_kwargs, return_dict=False)
